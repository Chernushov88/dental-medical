<template>
  <div class="relative">
    <div
      class="border border-gray-50 rounded-md px-6 py-4 bg-white relative flex items-center gap-2.5 cursor-pointer hover:bg-blue-light transition-all duration-300 z-10"
      @click="toggleOpen()"
    >
      ...
      <div class="w-6 relative">
        <div
          ...
          :class="
            isOpen ? 'rotate-45 -translate-y-1' : '-rotate-45 translate-y-1'
          "
        />
    ...
    >
      <div
        ...
        :key="item.value"
        class="text-b1 font-medium cursor-pointer hover:bg-blue-light/50 px-4 py-1 border-b border-gray-50/50 last-of-type:border-none transition-all duration-300"
...
const selected = computed(
  () => props.options.find((item) => item.value === selectedValue.value) || '',
);
